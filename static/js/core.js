...
/* ──────────────────────────────────────────────────────────
   EXTERNAL SERVER PANEL  (detail.html / tv_detail.html / anime_detail.html)
   ────────────────────────────────────────────────────────── */
(function () {
    // Run after DOM is ready
    document.addEventListener("DOMContentLoaded", initExtServerPanel);

    function initExtServerPanel() {
        const iframe = document.getElementById("player");
        const panel  = document.getElementById("extServerPanel");
        const grid   = document.getElementById("extServerGrid");
        const btn    = document.getElementById("extServerBtn");
        const chevron = document.getElementById("extChevron");

        if (!iframe || !panel || !grid || !btn) return;

        // Read media info from iframe data-attributes
        const mediaId   = iframe.dataset.contentId;
        const mediaType = iframe.dataset.contentType;  // movie | tv | anime
        const season    = iframe.dataset.season  || "1";
        const episode   = iframe.dataset.episode || "1";
        ...
        const SERVERS = [
            {
                ...
                badge: "Recommended",
                build: () => {
                    if (mediaType === "movie")  return `https://vidsrc.me/embed/movie?tmdb=${mediaId}`;
                    if (mediaType === "anime")  return `https://vidsrc.me/embed/anime/${mediaId}/${episode}`;
                    return `https://vidsrc.me/embed/tv?tmdb=${mediaId}&season=${season}&episode=${episode}`;
                }
            },
            {
                name: "VidSrc.to",
                badge: "Alt",
                build: () => {
                    if (mediaType === "movie")  return `https://vidsrc.to/embed/movie/${mediaId}`;
                    if (mediaType === "anime")  return `https://vidsrc.to/embed/anime/${mediaId}/${episode}`;
            ...
            },
            {
                ...
                badge: "Alt",
                build: () => {
                    if (mediaType === "movie")  return `https://embed.su/embed/movie/${mediaId}`;
                    if (mediaType === "anime")  return `https://embed.su/embed/tv/${mediaId}/${season}/${episode}`;
            ...
            },
            {
                ...
                badge: "Alt",
                build: () => {
                    if (mediaType === "movie")  return `https://player.autoembed.cc/embed/movie/${mediaId}`;
            ...
            },
            {
                ...
                badge: "Alt",
                build: () => {
                    if (mediaType === "movie")  return `https://vidlink.pro/movie/${mediaId}`;
            ...
            },
            {
                ...
                badge: "Alt",
                build: () => {
                    if (mediaType === "movie")  return `https://multiembed.mov/?video_id=${mediaId}&tmdb=1`;
                    return `https://multiembed.mov/?video_id=${mediaId}&tmdb=1&s=${season}&e=${episode}`;
                }
            }
        ];

        let activeIndex = 0; // vidsrc.me is default
        ...
        function buildGrid() {
            ...
            SERVERS.forEach((srv, i) => {
                ...
                b.className = "ext-server-btn" + (i === activeIndex ? " active" : "");
                b.innerHTML = `<span>${srv.name}</span><span class="srv-badge">${srv.badge}</span>`;
                b.addEventListener("click", () => {
                    activeIndex = i;
                    iframe.src = srv.build();
                    buildGrid();          // re-render to update active state
                    closePanel();
                    if (typeof showToast === "function") showToast(`Switched to ${srv.name}`, "success");
                });
                grid.appendChild(b);
            });
        }
        ...
        window.toggleExtServers = function (e) {
            e.stopPropagation();
