...
def create_app():
    ...
    def load_active_profile():
        ...
        if profile_id and g.current_user:
            try:
                profile = Profile.query.get(int(profile_id))
                # Only allow profile if it belongs to current user
        ...
        # Auth gates: if no user logged in, only allow landing/login/register
        auth_free = ['main.landing', 'main.login', 'main.register', 'api.suggest']
        if not g.current_user and request.endpoint and request.endpoint not in auth_free:
            return redirect(url_for('main.landing'))
        
        # If user logged in but no profile active and route is NOT in allowed_endpoints, redirect to profiles
        if g.current_user and not g.current_profile and request.endpoint and request.endpoint not in allowed_endpoints:
