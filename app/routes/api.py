...
def search_history():
    ...
    if request.method == "GET":
        history = db.session.query(SearchHistory).filter_by(profile_id=profile.id).order_by(SearchHistory.searched_at.desc()).limit(10).all()
        ...
        db.session.query(SearchHistory).filter_by(profile_id=profile.id).delete()
        db.session.commit()
        return jsonify({"success": True})