...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Priority order: vidsrc.me -> vidsrc.to -> embed.su -> autoembed -> vidlink -> multiembed
    ...
    const toggles = [
        [serverSelectorBtn, serverDropdownMenu],
        [subtitleSelectorBtn, subtitleDropdownMenu],
        [speedSelectorBtn, speedDropdownMenu]
    ];
    ...
    function buildServerDropdown() {
        if (!serverDropdownMenu || !iframe) return;
        serverDropdownMenu.innerHTML = "";

        serverPriority.forEach((server, index) => {
            ...
            btn.addEventListener("click", () => {
                ...
                const url = server.build(mediaId, currentSeason, currentEpisode);
                iframe.src = url;

                serverDropdownMenu.querySelectorAll("button").forEach(b => b.classList.remove("active"));
    ...
    function buildSubtitleDropdown() {
        ...
        const subtitles = ["Off", "English", "Hindi", "Telugu", "Tamil"];
        subtitleDropdownMenu.innerHTML = "";

        subtitles.forEach((lang, index) => {
            const btn = document.createElement("button");
            btn.textContent = lang;
            if (index === 0) btn.classList.add("active");

            btn.addEventListener("click", () => {
                subtitleDropdownMenu.querySelectorAll("button").forEach(b => b.classList.remove("active"));
                btn.classList.add("active");
                selectedSubLanguage = lang;
                showToast(`Subtitles: ${lang}`, "info");
            });
        ...
        if (speedDropdownMenu) {
            speedDropdownMenu.querySelectorAll("button").forEach(btn => {
                btn.addEventListener("click", () => {
                    speedDropdownMenu.querySelectorAll("button").forEach(b => b.classList.remove("active"));
                    btn.classList.add("active");
    ...
    function toggleFullscreen() {
        const playerContainer = document.querySelector(".player-wrap") || document.documentElement;
    ...
    if (playerWrapper) {
        ...
        // disabled auto-hide on mouse leave for better usability
    }
...
// Improved fullscreen + mobile orientation
if (fullscreenBtn) {
    fullscreenBtn.addEventListener("click", async () => {
        try {
            const elem = document.querySelector(".player-wrap");
            if (!document.fullscreenElement) {
...
// safer subtitle dropdown
if (subtitleSelectorBtn && subtitleDropdownMenu) {
    subtitleSelectorBtn.addEventListener("click", (e) => {
        e.preventDefault();
        e.stopPropagation();
        subtitleDropdownMenu.classList.toggle("open");
        subtitleDropdownMenu.style.display = subtitleDropdownMenu.classList.contains("open") ? "block" : "none";
    });
}

document.addEventListener("click", () => {
    if (subtitleDropdownMenu) {
        subtitleDropdownMenu.classList.remove("open");
        subtitleDropdownMenu.style.display = "none";
    }
});
