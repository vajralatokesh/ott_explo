from flask import Blueprint, render_template, redirect, url_for, request, make_response, g, jsonify, flash, session
from app.models import db, Profile, User, Watchlist, ContinueWatching, RecentlyWatched
...
@main_bp.route("/")
def landing():
    # If already logged in and has an active profile, go to dashboard
...
def profiles():
    ...
    all_profiles = Profile.query.filter_by(user_id=g.current_user.id).all()
...
def create_profile():
    ...
    # Check limit of 5 profiles per user
    if Profile.query.filter_by(user_id=g.current_user.id).count() >= 5:
        flash("You can have a maximum of 5 profiles.", "error")
        return redirect(url_for('main.profiles'))
        
    new_profile = Profile(name=name, avatar=avatar, is_kids=is_kids, user_id=g.current_user.id)
    if pin:
    ...
    # Autoselect profile if first profile for this user
    user_profile_count = Profile.query.filter_by(user_id=g.current_user.id).count()
    response = make_response(redirect(url_for('main.dashboard')))
    if user_profile_count == 1:
        response.set_cookie('active_profile_id', str(new_profile.id), max_age=31536000)
        return response
...
def select_profile(profile_id):
    if not g.current_user:
        return redirect(url_for('main.landing'))
    profile = Profile.query.filter_by(id=profile_id, user_id=g.current_user.id).first_or_404()
...
def dashboard():
    ...
    if profile.is_kids:
        ...
        popular_movies = fetch_movies().get("results", [])[:15]
        popular_tv = fetch_tv_shows().get("results", [])[:15]
...
def search():
    ...
    if q:
        ...
        existing = db.session.query(SearchHistory).filter_by(profile_id=profile.id, query=q).first()
        if existing:
        ...
        all_logs = db.session.query(SearchHistory).filter_by(profile_id=profile.id).order_by(SearchHistory.searched_at.asc()).all()
        if len(all_logs) > 10:
